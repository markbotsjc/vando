...
module.exports = function (client) {
    ...
    client.on("guildMemberAdd", async member => {
      ...
      if (textString3.length >= 14) {
        ...
        ctx.fillText(textString3, 800, canvas.height / 2 + 1);
      }
      //else dont do it
      else {
      ...
      ctx.fillStyle = '#f2f2f2';
      ctx.fillText(textString2, 1640, canvas.height / 2 + 58);
      //define the Member count
      var textString4 = `Se tornou um Piveti Agora` ;
      ctx.font = 'bold 60px "Roboto"';
      ctx.fillStyle = '#f2f2f2';
      ctx.fillText(textString4, 950, canvas.height / 2 + 125);
      //get the Guild Name
      var textString4 = `${member.guild.name}`;
      ctx.font = 'bold 60px "Roboto"';
      ctx.fillStyle = '#f2f2f2';
      ctx.fillText(textString4, 1050, canvas.height / 2 - 150);
      ...
      const welcomeembed = new Discord.MessageEmbed()
        .setColor("RANDOM")
        .setTimestamp()
        .setFooter("Entrou", member.guild.iconURL({ dynamic: true }))
        .setDescription(`**Bem Vindo(a) a ${member.guild.name}, Humano.**
      Ei <@${member.id}>! Leia as Regras! `)
